...
class ModeloStock{
	...
	static public function mdlMostrarStock($tabla, $item, $valor){
        ...
        "SELECT T0.CODIGO_INVENTARIO, T2.NOMBRE_GENERICO, T3.PRESENTACION, T4.TIPO_PRODUCTO, T2.STOCK_MIN, T0.CANTIDAD_PRODUCTO
          	FROM INVENTARIO T0
          ...
          AND $item = $valor"
			...
			$stmt = Conexion::conectar()->prepare("SELECT T0.CODIGO_INVENTARIO, T2.NOMBRE_GENERICO, T3.PRESENTACION, T4.TIPO_PRODUCTO, T2.STOCK_MIN, T0.CANTIDAD_PRODUCTO
          FROM INVENTARIO T0
          JOIN ASIGNACION_PRODUCTO T1 ON T0.CODIGO_ASIGNACION = T1.CODIGO_ASIGNACION
          JOIN PRODUCTO T2 ON T1.CODIGO_PRODUCTO = T2.CODIGO_PRODUCTO
          JOIN PRESENTACION T3 ON T1.CODIGO_PRESENTACION = T3.CODIGO_PRESENTACION
